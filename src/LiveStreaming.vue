<template>
    <section class="ly_broadcasting_area" v-if="isHomePage && user && isLive && liveData">
...
<!--                                                        <p v-if="!comment.sent_reply" class="reply"  @click.prevent="replyClickEvent(comment)"><i class="fas fa-reply"></i></p>-->
                                                    </div>
                                                </div>
                                        ...
                                        <div class="pinned_post" v-if="pinComments.length">
                                            <li v-for="(comment, i) in pinComments" :key="'pin_comment_dt_'+i">
                                                <div class="text">
                                                    <h2>{{ comment.name }}</h2>
                                                    <p>{{ comment.comment }}</p>
                                                </div>
                                                <div class="right_text">
                                                    <div class="pinned_icons">
                                                        <img src="/images/pin.png" alt="" class="pinned_icon">
                                                    </div>
                                                </div>
                                            </li>
                                        </div>
                            ...
                            <div class="reply_chat">
                                <div class="name">
                                    ...
                                    <img v-else :src="$config.BROADCAST_API+'/images/avatar.jpg'" alt="">
                                </div>
                                ...
                                <div class="imojies">
                                    ...
                                    <span @click.prevent="directImojiReact('LOVE')"><img :src="$config.BROADCAST_API+'/images/love2.png'" alt=""></span>
                                    ...
                                    <div class="view_product_moblie_wrap">
                                        ...
                                        <div class="inner_content product_details_list items1" id="Items1">
                                            ...
                                            <div v-if="currentItem && currentItem.inventories" :class="['pack_ratio', {scroll:currentItem.inventories.length > 3}]">
                                                <div class="row" v-for="inv in currentItem.inventories" :class="{active:activeColor === inv.color_id}" :key="'row_'+inv.id">
                                                                    ...
                                                                    <template v-if="user">
                                                                        <div class="num_count" v-if="inv.availability !== '2'">
                                                                            ...
                                                                            <div id="add" class="add">
                                                                                <button class="btn btn-default" @click.prevent="()=>{cartForm.colors.find((c) => c.id === parseInt(inv.color_id)), activeColor = parseInt(inv.color_id) }">
                                            ...
                                            <div class="pack_ratio_total_wrap">
                                                <div class="row">
                                                    ...
                                                    <div class="cms_col pl_5">
                                                        <div class="pack_ratio_total d_flex_btwn"><span>Total Price</span> <span>${{ totalPrice }}</span></div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="product_list_wrap">
                                                <ul>
                                                    <li v-for="(item, i) in products" @click.prevent="loadItem(item.slug, '#Items1')" :key="'mob_'+i">
                                                        <div class="product_inner_list" v-if="item">
                                                            <div class="img">
                                                            ...
                                                            <div class="text">
                                                                ...
                                                                <p>Price : ${{ item.price?  item.price.toFixed(2) : null}}</p>
                                                            </div>
...
<!--                                                                <p v-if="!comment.sent_reply" class="reply"  @click.prevent="replyClickEvent(comment)"><i class="fas fa-reply"></i></p>-->
                                                        ...
                                                        <div class="reply_comment" v-if="comment.replies && comment.replies.length">
                                                            <div class="reply_comment_list" v-for="reply in comment.replies" :key="'reply_mob_'+reply.id">
                                                                <div class="text">
                                                                    <h2>{{ reply.name }}</h2>
                                                                    <p>{{ reply.comment }}</p>
                                                                </div>
                                                ...
                                                <div class="pinned_post" v-if="pinComments.length">
                                                    <li v-for="(comment, i) in pinComments" :key="'pin_comment_'+i">
                                                        <div class="text">
                                                            <h2>{{ comment.name }}</h2>
                                                            <p>{{ comment.comment }}</p>
                                                        </div>
...
export default {
    ...
    methods: {
        getLiveStatus(){
            ...
            pubnub.subscribe({
                channels: [this.$config.VENDOR_ID],
                withPresence: true,
            });
        },
        getLiveVideo() {
            ...
            this.axios.get(this.$config.BROADCAST_API + '/api/current-live/' + this.$config.VENDOR_ID)
                .then((response) => {
                    if (response.data) {
                        ...
                        this.visitors = this.liveData.total_view;
                        if(this.user) {
                            this.isLive = true;
                            this.loadProducts();
                            this.loadComments();
                            ...
                            this.smsListScroll();
                            window.$("#live_notice_section").hide()
                        }else{
                            if(this.liveData && this.isLive){
                                window.$("#live_notice_section").show()
                            }
                        }
                    } else {
        ...
        directImojiReact(content){
            ...
            this.axios.post(this.$config.BROADCAST_API + '/api/comments/react-create', form)
        ...
        itemLoadFromDb(slug){
            this.axios.get(this.$config.WEB_API + '/api/item/' + slug)
                .then((response) => {
        ...
        addToCart() {
            if (this.cartForm.colors.reduce((a, b) => a + b.quantity, 0) === 0) {
                ...
                if (cartItems.length) {
                    this.axios.post(this.$config.WEB_API + '/api/live-cart/add', {items: cartItems}).then((response) => {
            ...
            if(this.liveData) {
                setTimeout(() => {
                    ...
                    this.player.on('playing', () => {
                        window.$('.custom_play_btn').hide();
                    });
                    this.player.on('waiting', () => {
                        window.$('.custom_play_btn').hide();
                    });
                    this.player.on('pause', () => {
                        window.$('.custom_play_btn').show();
                    });
                    this.player.on('pl', () => {
                        window.$('.custom_play_btn').show();
                    });
